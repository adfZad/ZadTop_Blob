...
namespace ZadHolding.Pager
{
    [ToolboxData("<{0}:ZadPager runat=\"server\"></{0}:ZadPager>")]
    public class ZadPager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        private string RenderGoToScript()
        {
            ...
            sb.AppendFormat("gotoImg.src = gotoElem.style.display == 'none' ? '{0}' : '{1}';",
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "ZadHolding.Pager.Images.arr_right.gif"),
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "ZadHolding.Pager.Images.arr_left.gif")
                                    );
            sb.Append("}");
